...
// Referenced from https://github.com/lurk-lab/bellpepper-gadgets/blob/main/crates/uint64/src/lib.rs
...
impl UInt64 {
        ...
        M: ConstraintSystem<Scalar, Root = MultiEq<Scalar, CS>>,
    {
        ...
        assert!(
            Scalar::NUM_BITS >= 64,
            "Assertion failed: Number of bits required to represent a field element should not be less than 64"
        );
        assert!(
            operands.len() >= 2,
...
mod test {
    ...
    fn test_uint64_sha512_maj() {
        let mut rng: XorShiftRng = XorShiftRng::from_seed([
            0x59, 0x62, 0xbe, 0x5d, 0x76, 0x3d, 0x31, 0x8d, 0x17, 0xdb, 0x37, 0x32, 0x54, 0x06,
            0xbc, 0xe5,
        ]);
