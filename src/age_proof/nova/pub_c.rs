use crate::utils::{
    ...
    conditionally_select_vec, less_than, less_than_or_equal, num_to_bits,
};
use crate::{
    dob::{
    ...
    pub fn new_state_sequence(
        ...
        for i in 1..num_steps {
            ...
            for k in 0..64 {
                ...
                let flag_coeff_c_less_2h = if c.coeff()[idx] < prod_s2h.coeff()[idx] {
                    true
                } else {
                    false
                };
            ...
            // prev_nullifier = Self::update_nullifier(prev_nullifier, l2_norm_sum, ctx_absorb, &falcon_msg);
            prev_nullifier =
                Self::update_nullifier(prev_nullifier, &msg_blocks[i % num_blocks], false);
        }
    ...
    Scalar: PrimeFieldBits + PartialOrd,
{
    ...
    fn synthesize<CS: ConstraintSystem<Scalar>>(
        ...
        let coeff_index_after =
            coeff_index_init.add(cs.namespace(|| "coeff_index + 64"), &var_64)?;
