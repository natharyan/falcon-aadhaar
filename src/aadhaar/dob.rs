// Referenced from nova-aadhaar-qr crate, modified to handle 136 byte blocks of qr code data
...
mod test {
    ...
    fn test_shift_left() {
        ...
        // 80-bit (10-byte) input
        // 1 << (shift_bits.len() + 2) == a.len() - 64
        // with shift_bits.len() == 2: 1 << 4 == 16 == 80 - 64
        let x_bytes: [u8; 10] = [0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22, 0x33, 0x44];
        let x_bits_values = bytes_to_bits(&x_bytes);
        let x_bits: Vec<Boolean> = x_bits_values
            .iter()
            .map(|&b| Boolean::Constant(b))
            .collect();

        for i in 0..4u32 {
            let shift_bytes = i as usize;

            // Expected: left shift by shift_bytes bytes (drop leading bits, pad zeros at end)
            let mut expected: Vec<Boolean> = x_bits[shift_bytes * 8..].to_vec();
            for _ in 0..shift_bytes * 8 {
                expected.push(Boolean::Constant(false));
            }

            // shift_bits in little-endian: bit 0 controls 1-byte shift, bit 1 controls 2-byte shift
            let mut shift_bits = u32_to_bits_le(i);
            shift_bits.truncate(2);

            let calc = left_shift_bytes(
            ...
            for j in 0..80 {
                assert_eq!(
                    expected[j].get_value().unwrap(),
                    calc[j].get_value().unwrap(),
                    "Mismatch at bit {j} for shift {i}"
                );
            }
        }
    }
