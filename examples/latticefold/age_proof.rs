...
use clap::Command;
use falcon_aadhaar::latticefold_adapter::ntt_pack::{num_lanes, pack_z};
use falcon_aadhaar::latticefold_adapter::utils::{
    alloc_public_z, negative_control, pad_lanes, synthesize_step, verify_both_sides,
};
...
use falcon_aadhaar::{
    ...
    // latticefold_adapter::lf_prove::{prove, verify},
    latticefold_adapter::ntt_pack::{
        check_field_relation, coefficient_norm_report, field_norm_report,
    },
    latticefold_adapter::shape_cs::{z_vector, BpMatrix},
    // latticefold_adapter::stark_field::StarkFq,
...
fn main() {
    ...
    println!("--- witness norm, field level (per-lane, before packing) ---");
    println!("{}", field_norm_report(&z_lanes));
    println!("--- witness norm, coefficient level (after icrt of z*) ---");
    println!("{}", coefficient_norm_report(&z_star));
