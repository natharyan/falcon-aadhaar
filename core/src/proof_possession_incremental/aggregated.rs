    ...
    Scalar: PrimeFieldBits + PartialOrd,
{
    ...
    pub fn new_state_sequence(
        ...
        for i in 1..8 {
            // let s2_subarray64 = s2.coeff()[coeff_index as usize..(coeff_index + 64u16) as usize].to_vec();
            // let c_subarray64 = c[coeff_index as usize..(coeff_index + 64u16) as usize].to_vec();
            // let prod_s2h_subarray64 = prod_s2h.coeff()[coeff_index as usize..(coeff_index + 64u16) as usize].to_vec();
            let mut sum_aggregated = 0u64;
            for k in 0..64 {
                let flag_coeff_c_less_2h = if c[coeff_index as usize] < prod_s2h.coeff()[coeff_index as usize] {true} else {false};
                // coefficients of both c and prod_s2h are already modulo q.
                let c_lt_s2h = c[coeff_index as usize] + MODULUS - prod_s2h.coeff()[coeff_index as usize];
                let s1_coeff = if flag_coeff_c_less_2h {c_lt_s2h} else {c[coeff_index as usize] - prod_s2h.coeff()[coeff_index as usize]};
