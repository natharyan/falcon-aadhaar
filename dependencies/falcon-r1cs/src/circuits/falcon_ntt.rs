...
impl<F: PrimeField> ConstraintSynthesizer<F> for FalconNTTVerificationCircuit {
    ...
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<()> {
        ...
        // NOTE s1 = c - s2*h
        // sig_poly(s) (created using s from sig = (nonce, s))
        // pk_poly(h) (created using h from pk = (h, A))
        ...
        // NOTE sig_poly_vars = s2 from the Falcon specification
        // NOTE create witness for each coefficient
        let sig_poly_vars =
        ...
        // NOTE create public input for each coefficient
        ...
        for e in v_vars.coeff() {
            // NOTE only constraint made on v is this and
            // ensure all the v inputs are smaller than MODULUS
            // v will need to be kept secret
            enforce_less_than_q(cs.clone(), &e)?;
        }
        // ========================================
        // NOTE both v and sig are private witnesses ane neither of them have been constrained
        // proving hm = v + sig * pk mod MODULUS
        // ========================================
