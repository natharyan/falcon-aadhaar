...
impl<F: PrimeField> ConstraintSynthesizer<F> for FalconSchoolBookVerificationCircuit {
    ...
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<()> {
        // NOTE = s2
        // LINK ../../../falcon-rust/src/structs/sig.rs#sig-to-poly
        let sig_poly: Polynomial = (&self.sig).into();
        // NOTE = h
        let pk_poly: Polynomial = (&self.pk).into();

        // NOTE no constraints created here, only a scalar factor for linear/non-linear operations between public inputs and(or) witnesses
        ...
        for e in sig_poly.coeff() {
            // NOTE Allocates a witness variable and assigns it a value, does not emit any R1CS constraints by itself.
            ...
            // ANCHOR a l2-norm that is at least > MODULUS^2 which is greater
        ...
        // pk
        //
        // NOTE here we use a buffer = [-pk[0], -pk[1], ..., -pk[N-1], pk[0], pk[1], ..., pk[N-1]]
        // where the buffer[i+1..i+513].reverse() will be used for the i-th column in inner-product calculation
        ...
        for e in pk_poly.coeff() {
            ...
            // NOTE only list of FpVars. this does not create new public inputs, not does it create constraints as let const_q_var = FpVar::<F>::new_constant(cs.clone(), F::from(MODULUS))?;
        ...
        // NOTE in total creates 28*512 = 14,336 constraints
        ...
        //
        //
        // NOTE build buffer = [-pk[0], -pk[1], ..., -pk[N-1], pk[0], pk[1], ..., pk[N]]
        ...
        for i in 0..N {
            ...
            let current_col: FpVar<F> = inner_product_mod(
                ...
                // NOTE x^N ≡ −1. The first half (−pk[..]) contributes the “wrapped” pk terms with the required negative sign
            ...
            // v = rhs mod MODULUS
            // NOTE creates 1 constraint
            (((&rhs).is_eq(&v_pos_vars[i])?)
                .or(&(&rhs).is_eq(&(&v_pos_vars[i] + &const_q_var))?)?)
            .enforce_equal(&Boolean::TRUE)?;

            // NOTE total constraints per iteration and  30 + 512 = 542
        }

        // ========================================
        // proving l2_norm(v | sig) < 34034726
        // ========================================
        // NOTE just allocates variables, does not create constraints
        let l2_norm_var = l2_norm_var(
            cs.clone(),
            &[v_pos_vars, sig_poly_vars].concat(),
            &const_q_var,
        )?;
        // NOTE creates 47 constraints.
